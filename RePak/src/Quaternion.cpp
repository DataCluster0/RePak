...
Vector3 Quaternion::ToEulerAngles() const
{
	...
	return Vector3(RadiansToDegrees(TempX), RadiansToDegrees(TempY), RadiansToDegrees(TempZ));
}
...
Quaternion Quaternion::Slerp(Quaternion& Rhs, float Time)
{
	...
	else
	{
		...
		if (fabs(sinTheta) < Epsilon)
		{
			...
			auto Tmp = Vector3(X, Y, Z).Lerp(0.5, Vector3(Rhs.X, Rhs.Y, Rhs.Z));
			ret.X = Tmp.x;
			ret.Y = Tmp.y;
			ret.Z = Tmp.z;
		}
...
Quaternion Quaternion::FromAxisRotation(Vector3 Axis, float Angle)
{
	...
	auto AngleResult = Vector3(Axis.x * AngleScale, Axis.y * AngleScale, Axis.z * AngleScale);
